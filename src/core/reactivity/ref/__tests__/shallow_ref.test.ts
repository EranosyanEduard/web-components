...
describe('тестовый набор утилиты `shallowRef`', () => {
  ...
  it('должен создать "поверхностно" реактивный объект', () => {
    ...
    const counters = shallowRef({ counterA: 0 })
    const processCounter = vi.fn<(counter: number) => void>()
  ...
  it('не должен создать реактивное значение из реактивного значения', () => {
    ...
    const counterA = shallowRef(0)

    expect(counterA).toBe(shallowRef(counterA))
  })

  it('не должен воспринимать реактивное значение в качестве реактивного объекта', () => {
    expect.hasAssertions()
    expect(isReactive(shallowRef(0))).toBe(false)
  })
})
